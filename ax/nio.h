...
public:
  ...
  int do_loop(int64_t timeout_us) {
    ...
    {
      Event event;
      if (0 != epoll_ctl(sock->epfd_, EPOLL_CTL_MOD, sock->fd_, make_epoll_event(&event, sock->get_event_flag(), id)))
      {
...
protected:
  ...
  int create_epoll_fd() {
    ...
    else if (AX_SUCCESS != (err = ev_queue_.push(make_epoll_event(event, 0, id))))
    {
      ERR(err);
    }
    if (AX_SUCCESS != err)
