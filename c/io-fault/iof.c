...
const char* _usages = "Usages: ./iof path err arg"
  "Example:\n"
  "\t./iof test\n"
  "\t./iof set ~/dir probability EAGAIN\n"
  "\t./iof set ~/dir probability TIMEOUT 10\n"
  "\t./iof set tcp:8000 probability TIMEOUT 10\n"
...
iof_t* get_iof_ctrl()
{
  ...
  return iof = (iof? :file_map(getenv("ctrl_file")?:DEFAULT_CTRL_FILE, sizeof(*iof)));
}
...
int iof_hook(int fd)
{
  ...
  else
  {
    while(!get_iof(iof, &pat))
     ...
     if (val.type == pat.type && val.dev_no == pat.dev_no && val.port == pat.port)
     {
