...
int64_t test_disk_latency(const char* path, int64_t sleep_us, int64_t write_size)
{
  ...
  int64_t file_size = 0;
  char file_path[256];
  int64_t write_ahead_size = 0;
  time_limit = atoll(getenv("time_limit")?:"3000000");
  file_size = atoll(getenv("file_size")?:"60");
  write_ahead_size = atoll(getenv("write_ahead_size")?:"0");
  printf("test_disk_latency(path=%s, sleep_us=%ld, write_size=%ld, write_ahead_size=%ld, time_limit=%ld, file_size=%ld)\n",
         path, sleep_us, write_size, write_ahead_size, time_limit, file_size);
  ...
  for(write_count = 0; 0 == err ; write_count++)
  {
    ...
    if (0 >= pwrite(fd, file_block, write_size + write_ahead_size, (write_count * write_size)  % (file_size*1024*1024)))
    {
      err = errno;
      perror("write");
    }
