...
namespace oceanbase
{
  ...
  {
    static const int64_t LOG_FILE_ALIGN_BIT = 12;
    ...
    int ObLogGenerator:: get_cursor(ObLogCursor& log_cursor) const
    {
      int err = OB_SUCCESS;
    ...
    int ObLogGenerator:: write_log(const LogCommand cmd, const char* log_data, const int64_t data_len)
    {
      ...
      else if (is_frozen_ || end_cursor_.offset_ + data_len + 2*LOG_FILE_ALIGN_SIZE > log_file_max_size_)
      {
        err = OB_BUF_NOT_ENOUGH;
               ...
               && OB_BUF_NOT_ENOUGH != err)
      {
        TBSYS_LOG(ERROR, "do_write_log(cmd=%ld, pos=%ld, len=%ld)=>%d", cmd, pos_, data_len, err);
      }
    ...
    int ObLogGenerator::switch_log()
    {
      ...
      {
        TBSYS_LOG(INFO, "switch_log(%ld)", end_cursor_.file_id_);
      }
      return err;
    }
