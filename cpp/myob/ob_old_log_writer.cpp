...
int ObLogWriter::write_log(const LogCommand cmd, const char* log_data, const int64_t data_len)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (!is_log_start_)
    {
      ret = OB_LOG_NOT_START;
      TBSYS_LOG(ERROR, "write_log(cmd=%d, log_data=%p, len=%ld): LOG_NOT_START", cmd, log_data, data_len);
    }
    ...
    if (log_buffer_.get_position() > 0)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret || OB_PARTIAL_FAILED == ret)
        {
          ...
          {
            TBSYS_LOG(ERROR, "first_entry.deserialize(buf=%p, buf_len=%ld, pos=0)=>%d",
                      log_buffer_.get_data(), log_buffer_.get_position(), ret);
          }
