...
// 只有在接下来要读的是log_entry header的时候才应该调用此函数
int ObSingleLogReader::trim_last_zero_padding(int64_t header_size)
{
  int err = OB_SUCCESS;
  bool enable_trim_zero = false;
  if (!enable_trim_zero)
  {}
  else if (!all_zero(log_buffer_.get_data() + log_buffer_.get_position(),
               min(log_buffer_.get_remain_data_len(), header_size)))
  {}
  else if (!all_zero(log_buffer_.get_data() + log_buffer_.get_position(),
                     log_buffer_.get_remain_data_len()))
  {
    TBSYS_LOG(ERROR, "header is zero, but not all remain data is zero. file=%ld, seq=%ld, offset=%ld",
              file_id_, last_log_seq_, pos);
    err = OB_INVALID_LOG;
  }
  else
  {
...
int ObSingleLogReader::read_header(ObLogEntry& entry)
{
  ...
  if ((OB_SUCCESS == err || OB_READ_NOTHING == err)
      && OB_SUCCESS != (trim_err = trim_last_zero_padding(entry.get_serialize_size())))
  {
    err = trim_err;
    TBSYS_LOG(ERROR, "trim_last_zero_padding()=>%d", err);
  }
  ...
  else if (log_buffer_.get_remain_data_len() < entry.get_serialize_size())
  {
    ...
    {
      TBSYS_LOG(WARN, "Get an illegal log entry, log file maybe corrupted");
  ...
  {
    err = OB_LAST_LOG_RUINNED;
    TBSYS_LOG(WARN, "Log entry deserialize error");
  }
  return err;
}
