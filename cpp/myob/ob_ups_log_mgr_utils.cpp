...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int scan_log_dir_func(const char* log_dir, int64_t& replay_point, int64_t& min_log_id, int64_t& max_log_id)
    {
      ...
      else if (OB_SUCCESS != (err = scanner.get_min_log_id((uint64_t&)min_log_id)) && OB_ENTRY_NOT_EXIST != err)
      {
        TBSYS_LOG(ERROR, "scanner.get_min_log_id()=>%d", err);
      }
      else if (OB_SUCCESS != (err = scanner.get_max_log_id((uint64_t&)max_log_id)) && OB_ENTRY_NOT_EXIST != err)
      {
        TBSYS_LOG(ERROR, "get_max_log_id error[ret=%d]", err);
      }
      ...
      else
      {
        ...
        if (0 >= replay_point)
        {
          replay_point = min_log_id;
        }
        if (min_log_id > replay_point || max_log_id >0 && replay_point > max_log_id)
        {
          err = OB_DISCONTINUOUS_LOG;
          TBSYS_LOG(WARN, "min_log_id=%ld, max_log_id=%ld, replay_point=%ld", min_log_id, max_log_id, replay_point);
        }
      }
    ...
    int replay_single_log_func(ObUpsMutator& mutator, CommonSchemaManagerWrapper& schema, ObUpsTableMgr* table_mgr, LogCommand cmd, const char* log_data, int64_t data_len)
    {
      int err = OB_SUCCESS;
      int64_t pos = 0;
      int64_t log_id;
    ...
    int replay_log_func(const volatile bool& stop, const char* log_dir, const ObLogCursor& start_cursor, const ObLogCursor& end_cursor, ObLogCursor& return_cursor, IObLogApplier* log_applier)
    {
      ...
      while (!stop && OB_SUCCESS == err)
      {
        ...
        else if (OB_SUCCESS == err && OB_SUCCESS != (err = log_reader.get_cursor(log_cursor)))
        {
        ...
        else if (OB_SUCCESS != (err = log_applier->apply_log(cmd, log_data, data_len)))
        {
    ...
    int replay_log_in_buf_func(const volatile bool& stop, const char* log_data, int64_t data_len, IObLogApplier* log_applier)
    {
      int err = OB_SUCCESS;
      ObLogEntry log_entry;
      int64_t pos = 0;
      TBSYS_LOG(INFO, "replay_log_in_buf_func(stop=%s, log_data=%p, data_len=%ld)", STR_BOOL(stop), log_data, data_len);
      while (!stop && OB_SUCCESS == err && pos < data_len)
      {
        if (OB_SUCCESS != (err = log_entry.deserialize(log_data, data_len, pos)))
        {
          TBSYS_LOG(ERROR, "log_entry.deserialize(log_data=%p, data_len=%ld, pos=%ld)=>%d", log_data, data_len, pos, err);
        ...
        else if (OB_SUCCESS != (err = log_applier->apply_log((LogCommand)log_entry.cmd_, log_data + pos, log_entry.get_log_data_len())))
        {
