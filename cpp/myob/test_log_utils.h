...
namespace oceanbase
{
  ...
  {
    template<typename T>
    int64_t get_max_log_id(const T& t)
    {
      int err = OB_SUCCESS;
      int64_t log_id = 0;
    ...
    template<typename T>
    int64_t get_max_file_id(const T& t)
    {
      int err = OB_SUCCESS;
      int64_t file_id = 0;
      ObLogCursor log_cursor;
    ...
    int gen_log_and_handle(IObBatchLogHandler* log_handler, ObLogGenerator& log_generator, const int64_t end_id) {
      ...
      while(OB_SUCCESS == err && get_max_log_id(log_generator) < end_id) {
        if (OB_SUCCESS != (err = gen_next_log(get_max_log_id(log_generator), single_log_buf, sizeof(single_log_buf), single_log_buf_len)))
        {
