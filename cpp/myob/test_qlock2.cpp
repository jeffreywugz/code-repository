  ...
  namespace test
  {
    ...
    struct Node
    {
      ...
      int check(int64_t uid){
        ...
        {
          err = OB_INVALID_DATA;
          TBSYS_LOG(ERROR, "check(uid=%ld): ERROR", uid);
        }
        if (OB_SUCCESS == err)
        {
      ...
      int set(int64_t uid, int64_t id){
        int err = OB_SUCCESS;
        int unlock_err = OB_EAGAIN;
        bool is_locked = true;
        __sync_fetch_and_add(&n_call_, 1);
        ...
        {
          TBSYS_LOG(ERROR, "lock.wait_shared_lock_release(uid=%ld)=>%d", uid, err);
        }
        else if (x != y)
        {
          err = OB_INVALID_DATA;
      ...
      int set2(int64_t uid, int64_t id){
        int err = OB_SUCCESS;
        int unlock_err = OB_EAGAIN;
          ...
          for (start_id = 0; !_stop && OB_SUCCESS == err && start_id < max_num_items; start_id++)
          {
            switch(random()%3)
            {
              case 0:
                ...
                break;
              case 1:
                err = node.set(idx, start_id);
                break;
              case 2:
                err = node.set2(idx, start_id);
                break;
              default:
